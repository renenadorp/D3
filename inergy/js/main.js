...
//SELECTIONS
var VISIBLE_RINGS 		= 4;
const selectRings    = document.querySelector('#rings');

selectRings.addEventListener('change', (event) => {
  VISIBLE_RINGS = event.target.value;
  chart(dsOrig)
});

//-- COLOUR CODE 
var COLOURCODE 		= "traffic";
const selectColourCode = document.querySelector('#selectColourCode');
selectColourCode.addEventListener('change', (event) => {
...
//-- COMPETITOR
var COMPETITOR 		      = "Inergy";
const selectCompetitor    = document.querySelector('#selectCompetitor');
selectCompetitor.addEventListener('change', (event) => {
	
	COMPETITOR = event.target.value;
	chart(dsOrig);
  });  

  const selectEnvironment  = document.querySelector('#selectEnvironment');
  selectEnvironment.addEventListener('change', (event) => {
	  
	  ENVIRONMENT = event.target.value;
	  LINK =`${BASEFILENAME}${ENVIRONMENT}.xlsx`;
	  chartData(LINK);
  });  
...
//-- IDs	
var TOGGLEID 		= false;
const toggleId = document.querySelector('#toggleId');
toggleId.addEventListener('change', (event) => {
	
	TOGGLEID = event.target.checked;
	//console.log('t:', event.target.checked)
	chart(dsOrig)
	});
...
function chart(ds) {
	...
	path = g.append("g")
		...
		.attr("fill", d => {
							const gradeColumn = `grade${COMPETITOR}`
							console.log('gc:', gradeColumn)
							const grade = +`${d.data.data[gradeColumn]}`
							const depth = d.depth;

							const cv = colorValue(COLOURCODE, grade, depth);
...
function setLegend(){
	...
	legendDiv.innerHTML ="";
	if (COLOURCODE =='coverage')
	legendDiv.innerHTML = 
	`<div class="input-group">

	<div class="input-group-prepend legend">
		<span class="input-group-text" id=""  >
		Legend
		</span>
	</div>
	<div class="form-control legend">
	<table class="legend">
	<tr>
	<td class="color" style="background-color: ${colorScaleCoverageOrdinal(-1)}"></td><td>Unknown</td>
	</tr>
	<tr>
	<td class="color" style="background-color: ${colorScaleCoverageOrdinal(0)}"></td><td>Not covered</td>
	</tr>
	<tr>
	<td class="color" style="background-color: ${colorScaleCoverageOrdinal(1)}"></td><td>Covered</td>
	</tr>
	</table>
	</div>
	</div>
	...
	`<div class="input-group">
		...
		<div class="form-control legend">
			<table class="legend">
				<tr>
				<td class="color" style="background-color: ${colorScaleTrafficOrdinal(-1)}"></td><td>Unknown</td>
				</tr>
				<tr>
				<tr>
				<td class="color" style=" background-color: ${colorScaleTrafficOrdinal(0)}"></td><td>Non Existent</td>
				</tr>
				<tr>
				<td class="color" style=" background-color: ${colorScaleTrafficOrdinal(1)}"></td><td>Initial</td>
				</tr>
	
				<tr>
				<td class="color" style=" background-color: ${colorScaleTrafficOrdinal(2)}"></td><td>Managed</td>
				</tr>
		
				<tr>
				<td class="color" style=" background-color: ${colorScaleTrafficOrdinal(3)}"></td><td>Defined</td>
				</tr>
	
		
				<tr>
				<td class="color" style=" background-color: ${colorScaleTrafficOrdinal(4)}"></td><td>Measured</td>
				</tr>
	

				<tr>
				<td class="color" style=" background-color: ${colorScaleTrafficOrdinal(5)}"></td><td>Optimized</td>
				</tr>
			</table>
		</div>	
	</div>
