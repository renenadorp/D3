...
export function drawCharacteristics(s, props, characteristics)  {
  const {width} = props; //Unpack parameters  
  console.log('sCharacter', characteristics)
  var sCharacteristics = d3.selectAll('.gCharacteristic').data(characteristics);
  sCharacteristics
      .join(
          function(enter){
            console.log('enter')
            var gCharacteristic = enter.append('text').text(d=>{console.log(d);return d.key})
